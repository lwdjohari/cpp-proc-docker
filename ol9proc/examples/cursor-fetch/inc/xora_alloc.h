...
extern "C"
{
   ...
   */
  static inline void xora_str_copy_bounded(char *dst, const char *src, size_t dst_size)
  {
    if (!dst || dst_size == 0)
      return;
  ...
  static inline char *xora_stralloc(const char *src)
  {
    if (!src)
      return NULL;
    size_t len = strlen(src) + 1; /* +1 for '\0' */
    char *dst = (char *)xora_malloc(len);
    if (!dst)
      return NULL;
    memcpy(dst, src, len);
    return dst;
  ...
  static inline char *xora_strnalloc(const char *src, size_t maxlen)
  {
    if (!src)
      return NULL;
    ...
    char *dst = (char *)xora_malloc(srclen + 1);
    if (!dst)
      return NULL;
  ...
  static inline size_t xora_strset(char *dst, size_t bufsize, const char *src)
  {
    if (!dst || bufsize == 0)
      return 0;
    if (!src)
  ...
  static inline size_t xora_strnset(char *dst, size_t bufsize,
                                  const char *src, size_t maxlen) {
    if (!dst || bufsize == 0) return 0;
 ...
 * int *a = XORA_ALLOC_ARRAY(int, 100);
 *
 * // Allocate array of 50 doubles, zero-initialized
 * double *b = XORA_CALLOC_ARRAY(double, 50);
 *
 ...
 *
 * emp_t *emps = NULL;
 * emps = XORA_ALLOC_ARRAY(emp_t, 100);       // initial alloc
 * emps = XORA_RESIZE_ARRAY(emps, emp_t, 200); // resize to 200
 *
 * // Always check result for NULL if your allocators can fail
 * if (!emps) {  handle out-of-memory  }
 * Don’t forget to free() via xora_free() when done
 */
...
#define XORA_STRSET(field, src) \
  xora_strset((field), sizeof(field), (src))

#define XORA_STRNSET(field, src, maxlen) \
    xora_strnset((field), sizeof(field), (src), (maxlen))
